...
data <- data %>%
  ...
  mutate(collectDate = case_when(
    n_distinct(collectDate) > 1 ~ Mode(collectDate), #need to fix Mode because we want the mode of the distinct grouping
    TRUE ~ collectDate
  )) %>%
  ungroup() %>%
...
bout_trap_days_old <- beetles_counts %>%
...
bout_trap_days <- beetles_counts %>%
  select(siteID, plotID, trapID, collectDate, trappingDays) %>%
  distinct() %>%
...
beetles_site_bout <- beetles_counts %>% 
  filter(!is.na(count)) %>%
  select(siteID, plotID, eventID, setDate, collectDate, scientificName, count) %>%
  group_by(siteID, setDate, collectDate, eventID, scientificName) %>%
  mutate(abundance = sum(count)) %>%
  select(-c(plotID, count)) %>%
  distinct() %>%
  ungroup() %>%
  left_join(bout_trap_days, by = c("siteID", "eventID")) %>%
  #some plots have a different eventID even though they were set and collected on the same day 
  #as other plots in the site, because those plots had another collection in the interim
  #so we really want to get counts for a collectDate, not an eventID
  group_by(siteID, collectDate, scientificName) %>%
  mutate(new_abundance = sum(abundance), new_totalTrapDays = sum(totalTrapDays))
