...
#For some reason there are ~10 individualID's for which experts ID'd more than one species
...
There are also quite a few which have more species identified than the individual count
```{r}
data_expert %>% 
    group_by(year, subsampleID, individualCount) %>%
    summarise(n_species = n_distinct(taxonID)) %>% 
    filter(n_species > individualCount) %>%
    View()
```
