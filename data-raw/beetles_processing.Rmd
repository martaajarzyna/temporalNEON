...
data <- beetles_raw$bet_fielddata %>%
  ...
  #Code below will take the original trappingDays entry where supplied, unfortunately it doesn't seem to account for traps that were
  #sampled more than once for a given setDate (always measures days from setDate, not last time collected), so we might as well just 
  #calculate our own the same way and not deal with NA's
  # #it seems number of trapping days is only entered where the eventID is recorded without period separators, 
  # #so let's make them all look the same
  # mutate(eventID = str_remove_all(eventID, "[.]")) %>%
  # #and then fill in NA's
  # group_by(eventID) %>%
  # fill(trappingDays) %>%
  # #there are still some eventID's that have no trapping days calculated (they're NA), so calculate those
  # mutate(calctrappingDays = interval(ymd(setDate), ymd(collectDate)) %/% days(1))#,
  #        newtrappingDays = case_when(
  #   is.na(trappingDays) ~ calctrappingDays,
  #   TRUE ~ trappingDays
  # ))
  #let's get number of trapping days for each bout, the trapping days doesn't really make sense %>%
...
bout_trap_days <- beetles_counts %>%
  select(siteID, plotID, trapID, eventID, trappingDays) %>%
  distinct() %>%
...
beetles_site_bout <- beetles_counts %>% 
  filter(!is.na(count)) %>%
  select(siteID, plotID, eventID, collectDate, scientificName, count) %>%
  #for some eventID's (bouts) collection happened over two days, change collectDate to just the first day
  #so we have a single collectionDate
  group_by(siteID, plotID, eventID) %>%
  mutate(collectDate = case_when(
    n_distinct(collectDate) > 1 ~ unique(collectDate)[1],
    TRUE ~ collectDate
  )) %>%
  group_by(siteID, collectDate, eventID, scientificName) %>%
  mutate(abundance = sum(count)) %>%
  select(-c(plotID, count)) %>%
  distinct() %>%
  ungroup() %>%
  left_join(bout_trap_days, by = c("siteID", "eventID")) 

#Inter annual variability

#calculate trap days in a year 
yearly_trap_days <- bout_trap_days %>%
  mutate(year = substr(eventID, 5,8)) %>%
...
codyn_beetles_inter <- beetles_site_bout %>%
  mutate(year = substr(eventID, 5,8)) %>%
  select(-c(totalTrapDays, eventID, collectDate)) %>%
