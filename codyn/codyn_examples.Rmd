...
fish_change_inter <- RAC_change(fish_inter, 
                             time.var = "year", species.var = "scientificName",
                             abundance.var = "abundance", replicate.var = "siteID")
...
Inverts
```{r}
# invert_inter <- read_csv("~/Documents/Projects/temporalNEON/data/inverts/inverts_table_abundance_year_BAT.csv") %>%
#   pivot_longer(-c(siteID, year), names_to = "scientificName", values_to = "abundance")
# 
# invert_intra <- read_csv("~/Documents/Projects/temporalNEON/data/inverts/inverts_table_abundance_month_BAT.csv") %>%
#   pivot_longer(-c(siteID, year, month, bout), names_to = "scientificName", values_to = "abundance")
# 
# invert_other <- read_csv("~/Documents/Projects/temporalNEON/data/inverts/inverts_table_abundance_codyn.csv")
# 
# invert_inter <- RAC_change(invert_inter, 
#                              time.var = "year", species.var = "scientificName",
#                              abundance.var = "abundance", replicate.var = "siteID")
# 
# invert_intra <- RAC_change(invert_intra, 
#                              time.var = "bout", species.var = "scientificName",
#                              abundance.var = "abundance", replicate.var = "siteID")
```
...
```{r}
taxa_names <- c("beetle", "mammal", "fish")

plot_inter_intra <- function(taxa_name, data = codyn_df){
  change_vars <- c("richness_change", "evenness_change", "rank_change", "gains", "losses")
  ...
  intra_v_inter <- data %>%
    ...
    ggplot(aes(inter_site_mean, intra))+
    geom_point(size=3)+
    geom_smooth(method="glm")+
    geom_point(aes(inter_site_mean, intra_site_mean), col="red", size=3)+
    geom_smooth(aes(inter_site_mean, intra_site_mean), col="red",method="glm")+
  ...
  intra_v_inter_means <- data %>%
    filter(taxa == taxa_name) %>%
    ggplot(aes(inter_site_mean, intra_site_mean))+
  ...
  plot <- ggarrange(intra_v_inter, intra_v_inter_means, nrow =2)
  ggsave(file=paste0(here::here("codyn/figures"), "/", taxa_name, "-intra-inter.jpeg"), plot, width=30,height=10, dpi=300)
}

map(taxa_names, plot_inter_intra)
```
